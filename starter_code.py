...
def compare_all_approaches():
    ...
    for size in sizes:
        ...
        # Print results
        rec_str = (
            f"{times['recursive']:.4f}s"
            if times["recursive"] is not None
            else "Too slow"
        )
        mem_str = (
            f"{times['memoization']:.4f}s"
            if times["memoization"] is not None
            else "ERROR"
        )
        tab_str = (
            f"{times['tabulation']:.4f}s"
            if times["tabulation"] is not None
            else "ERROR"
        )
